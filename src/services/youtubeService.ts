...
export class YouTubeService {
    ...
    /**
     * Get API key from VS Code settings or .env file
     * Priority: VS Code settings > .env file > environment variable
     */
    private getApiKey(): string {
        ...
        // 3. Check process environment variable
        if (process.env.YOUTUBE_API_KEY) {
            this.log('Using API key from environment variable');
            return process.env.YOUTUBE_API_KEY;
        }

        throw new Error('YouTube API key not configured. Set youtubeMcp.apiKey in settings or add YOUTUBE_API_KEY to .env file.');
    }
